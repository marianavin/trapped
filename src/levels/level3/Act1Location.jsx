...
export default function Act1Location({ onProceed }) {
  ...
  return (
    <div className="h-full w-full flex flex-col bg-l3-bg px-5 py-8 sm:px-10">
      <div className="font-pixel text-[10px] text-l3-prompt">
        LOCATION: <span className="text-white">{address ?? '—'}</span>
      </div>

      <div className="flex-1 flex flex-col items-center justify-center gap-8">
        <div className="w-full max-w-sm border-2 border-white/70 bg-black/30 px-4 py-4">
        ...
        <AnimatePresence>
          {signVisible && (
            <motion.button
              ...
              className={`font-mono text-sm sm:text-base px-4 py-3 border-2 w-full max-w-sm text-left
